        ...
        <Stack direction={direction} spacing={spacing}>
          {badges.map((badgeProp) => {
            return (
              <StyledBadge
                {...props}
                variant={badgeProp.variant || variant}
                overlap={badgeProp.overlap || overlap}
                anchorOrigin={badgeProp.anchorOrigin || anchorOrigin}
                sx={badgeProp.sx || sx}
                color={badgeProp.color || color}
              >
          ...
          {badges.map((badgeProp) => {
            return (
              <NMSBadge
                {...props}
                variant={badgeProp.variant || variant}
                max={badgeProp.max || max}
                color={badgeProp.color || color}
                sx={badgeProp.Badgesx || Badgesx}
                badgeContent={badgeProp.badgeContent || badgeContent}
                overlap={badgeProp.overlap || overlap}
              >
                <IconComponent
                  size={badgeProp.size || size}
                  iconStyle={badgeProp.iconStyle || iconStyle}
                  sx={badgeProp.Iconsx || Iconsx}
                />
              </NMSBadge>
            );
          })}
